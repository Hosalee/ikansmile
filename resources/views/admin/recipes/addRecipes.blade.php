...
<main class="col bg-faded py-4 flex-grow-1">
    ...
    <div class="container mt-3">
        <div class="row">
            ...
            <form action="{{-- route('storeRawMaterial') --}}" method="POST" enctype="multipart/form-data" class=" p-5">
                @csrf
                <div class="row">
                    <div class="col-md-4 ">
                        <div class="form-group my-3">
                            <label for="picture">รูปสูตรอาหาร</label>
...
<script type="text/javascript">
    ...
    $("#dynamic-ar").click(function () {
        ...
        $("#dynamicAddRemove").append(
            '<tr><td></td><td><select class="custom-select my-1 mr-sm-2" aria-label="Default select example" name="RM_id[]" placeholder="Enter subject"><option >@foreach($RM as $row) <option  value="{{$row->Raw_Material_id}}">{{$row->Raw_Material_name}}</option>@endforeach </option></select></td>  <td><input type="text" name="Quantity[]" placeholder="ระบุจำนวน" class="form-control mt-1" /></td> <td><button type="button" class="btn btn-danger remove-input-field">Delete</button></td></tr>');
    });
